...
const getAddressBalance = addresses => new Promise((resolve, reject) => {
  client.getaddressbalance(addresses, (err, r) => {
    if (err) {
      reject(new DashCoreRpcError(err.message, null, err.code));
    } else {
      resolve(r.result);
...
const getTransactionFee = (addresses, amount) => new Promise((resolve, reject) => {
  client.gettransactionfee(addresses, amount, (err, r) => {
    if (err) {
      reject(new DashCoreRpcError(err.message, null, err.code));
    } else {
      resolve(r.result);
 ...
 * getTransactionFee: (function(): Promise<unknown>)}}
 */
module.exports = {
  generateToAddress,
  getBestBlockHash,
