const RpcClient = require('dashd-rpc');
...
const getTransactionFee = (address, amount) => new Promise((resolve, reject) => {
  client.gettransactionfee(address, amount, (err, r) => {
    if (err) {
      reject(new DashCoreRpcError(err.message, null, err.code));
    } else {
      resolve(r.result);
...
const getAddressBalance = address => new Promise((resolve, reject) => {
  client.getaddressbalance(address, (err, r) => {
    if (err) {
      reject(new DashCoreRpcError(err.message, null, err.code));
    } else {
      resolve(r.result);
...
const getAddressTransactionsIds = address => new Promise((resolve, reject) => {
  client.getaddresstxids(address, (err, r) => {
    if (err) {
      reject(new DashCoreRpcError(err.message, null, err.code));
    } else {
      resolve(r.result);
 ...
 * getAddressBalance: (function(): Promise<unknown>),
 * getTransactionFee: (function(string, number): Promise<number>),
 * getAddressTransactionsIds: (function(): Promise<unknown>)}}
 */
module.exports = {
  generateToAddress,
  getBestBlockHash,
